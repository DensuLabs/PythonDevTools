...
class AesEncryption(object):
    ...
    def __init__(self, mode='CBC', size=128):
        self._modes = AesModes
        self._sizes = (128, 192, 256)
        self._salt_length = 16
        self._iv_length = 16
        self._mac_length = 32
        self._mac_key_length = 32

        if mode.upper() not in self._modes.__members__:
            raise ValueError(f'{mode} is not supported!')
        if size not in self._sizes:
            raise ValueError('Invalid key size!')

        self._mode = self._modes[mode.upper()]
        self._key_length = size // 8
    ...
    def encrypt_file(self, path, password=None):
        ...
        try:
            ...
            with open(encrypted_path, "wb") as encrypted_file:
                ...
                encrypted_file.write(hmac.digest())
    ...
    def decrypt_file(self, path, password=None):
        ...
        try:
            with open(path, 'rb') as encrypted_file:
                salt = encrypted_file.read(self._salt_len)
                iv = encrypted_file.read(self._iv_len)
                encrypted_file.seek(file_size(path) - self._mac_len)
                mac = encrypted_file.read(self._mac_len)

            aes_key, mac_key = self._keys(salt, password)
            self._verify_file(path, mac, mac_key)
            cipher = self._cipher(aes_key, iv)
            decrypted_path = re_sub(r'\.enc$', '.dec', path)
            ...
            with open(decrypted_path, 'wb') as decrypted_file:
                ...
                if self._mode == AesModes.CBC and is_last:
    ...
    def get_master_key(self, raw=False):
        ...
        if self._master_key is None:
            self._handle_key_not_set_error()

        return self._master_key if raw else b64encode(self._master_key)
    ...
    def _file_chunks(self, path, beg=0, end=0, chunk_size=1024):
        '''
        ...
        with open(path, 'rb') as f:
            f.seek(beg)
            remaining_size = file_size(path) - beg - end

        while remaining_size > 0:
            size = min(chunk_size, remaining_size)
            data = f.read(size)
            remaining_size -= size

            yield (data, remaining_size == 0)
